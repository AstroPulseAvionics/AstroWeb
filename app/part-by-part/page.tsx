import React, { Suspense } from "react";
import PartByPart from "@/components/part-by-part";
import ContactUs from "@/components/contactUs";
import {
  getGeneralFunding,
  getGeneralProjectCost,
  getParts,
} from "@/lib/dynamodb";

export const dynamic = "force-dynamic";

export default async function PartByPartPage() {
  const [parts, generalFunding, generalProjectCost] = await Promise.all([
    getParts(),
    getGeneralFunding(),
    getGeneralProjectCost(),
  ]);

  return (
    <main className="relative flex flex-col items-center justify-center">
      <div className="pointer-events-none absolute top-0 -z-10 h-screen w-screen max-w-full bg-[radial-gradient(circle_at_top,_rgba(255,95,31,0.1),_transparent_60%)] bg-[length:140%_140%] bg-[position:50%_0%] sm:bg-[length:120%_120%]" />
      <PartByPart
        parts={parts}
        generalFunding={generalFunding}
        generalProjectCost={generalProjectCost}
      />
      <Suspense fallback={null}>
        <ContactUs disableSectionTracking />
      </Suspense>
    </main>
  );
}
